    ...
    public class AccountController : Controller
    {
        ...
        public IActionResult GetProfileFriend(string id)
        {
            using (var db = new Context())
            {
                ...
                var followCount = db.Follows.Where(i =>i.BuyerToken == id && i.Status==1).Count();
        ...
        public IActionResult Friends()
        {
            ...
            using (var db = new Context())
            {
                ...
                List<User> Followsss = new List<User>();

                foreach (var a in Follows)
                {
                    var x = _userManager.Users.Where(i => i.Id == a.SenderToken).FirstOrDefault();
                ...
                List<User> Followss = new List<User>();

                foreach (var a in Follow)
                {
                    var x = _userManager.Users.Where(i => i.Id == a.BuyerToken).FirstOrDefault();
